...
// Package serviceaccount implements the Crossplane managed reconciler for the
// Anthropic Admin API ServiceAccount resource.
...
const (
	errNotServiceAccount  = "managed resource is not a ServiceAccount"
	errNewClient = "cannot build Anthropic client"
	errObserve   = "cannot observe ServiceAccount"
	errCreate    = "cannot create ServiceAccount"
	errUpdate    = "cannot update ServiceAccount"
	errDelete    = "cannot archive ServiceAccount"
)

// Setup adds a controller for ServiceAccount to the supplied manager.
func Setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1beta1.ServiceAccountKind)
	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		For(&v1beta1.ServiceAccount{}).
		Complete(managed.NewReconciler(mgr,
			resource.ManagedKind(v1beta1.ServiceAccountGroupVersionKind),
...
// SetupGated registers the ServiceAccount controller to start only once the
// ServiceAccount CRD is established.
func SetupGated(mgr ctrl.Manager, o controller.Options) error {
	o.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			panic(err)
		}
	}, v1beta1.ServiceAccountGroupVersionKind)
	return nil
}
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	sa, ok := mg.(*v1beta1.ServiceAccount)
	if !ok {
		return nil, xperrors.New(errNotServiceAccount)
	}

	cl, err := clients.NewClient(ctx, c.kube, sa)
	if err != nil {
...
// external implements managed.ExternalClient for Anthropic ServiceAccounts.
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	sa, ok := mg.(*v1beta1.ServiceAccount)
	if !ok {
		return managed.ExternalObservation{}, xperrors.New(errNotServiceAccount)
	}

	// Crossplane seeds external-name with the k8s object name before Create runs.
	id := meta.GetExternalName(sa)
	if id == "" || id == sa.GetName() {
	...
	sa.FromAnthropicObservation(*resp)

	sa.SetConditions(xpv2.Available())

	return managed.ExternalObservation{
		ResourceExists:   true,
		ResourceUpToDate: isUpToDate(sa),
	}, nil
}
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	meta.SetExternalName(sa, resp.ID)
	sa.Status.AtProvider.ID = &resp.ID
...
func (e *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	sa, ok := mg.(*v1beta1.ServiceAccount)
	if !ok {
		return managed.ExternalUpdate{}, xperrors.New(errNotServiceAccount)
	}
...
func (e *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	sa, ok := mg.(*v1beta1.ServiceAccount)
	if !ok {
		return managed.ExternalDelete{}, xperrors.New(errNotServiceAccount)
	}
...
func isUpToDate(sa *v1beta1.ServiceAccount) bool {
	fpRaw, err := json.Marshal(sa.Spec.ForProvider)
	if err != nil {
		return true
	}
	apRaw, err := json.Marshal(sa.Status.AtProvider)
	if err != nil {
		return true
	}
