...
func Setup(mgr ctrl.Manager, o controller.Options, skipDefaultMetadata bool) error {
	name := managed.ControllerName(v1beta1.MemoryStoreKind)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1beta1.MemoryStore{}).
		Complete(managed.NewReconciler(mgr,
			resource.ManagedKind(v1beta1.MemoryStoreGroupVersionKind),
			opts...,
		))
}
...
func SetupGated(mgr ctrl.Manager, o controller.Options, skipDefaultMetadata bool) error {
	...
	}, v1beta1.MemoryStoreGroupVersionKind)
	return nil
}
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	ms.SetConditions(xpv2.Available())

	return managed.ExternalObservation{
		ResourceExists:   true,
		ResourceUpToDate: isUpToDate(ms),
	}, nil
}
...
func isUpToDate(ms *v1beta1.MemoryStore) bool {
	fpRaw, err := json.Marshal(ms.Spec.ForProvider)
	if err != nil {
		return true
	}
	apRaw, err := json.Marshal(ms.Status.AtProvider)
	if err != nil {
		return true
	}
